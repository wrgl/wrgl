...
func TestDetectMovedColumns(t *testing.T) {
	assert.Equal(t, detectMovedColumns([]*column{{name: "a"}}, []string{"a"}), []*column{
		{name: "a"},
	})
	assert.Equal(t, detectMovedColumns([]*column{{name: "a"}}, []string{"b"}), []*column{
		{name: "a"},
	})
	assert.Equal(t,
		detectMovedColumns([]*column{{name: "a"}, {name: "b"}}, []string{"b"}),
		[]*column{{name: "a"}, {name: "b"}})
	assert.Equal(t,
		detectMovedColumns([]*column{{name: "a"}, {name: "b"}}, []string{"a"}),
		[]*column{{name: "a"}, {name: "b"}})
	assert.Equal(t,
		detectMovedColumns([]*column{{name: "a"}, {name: "b"}}, []string{"a", "b"}),
		[]*column{{name: "a"}, {name: "b"}})
	assert.Equal(t,
		detectMovedColumns([]*column{{name: "a"}, {name: "b"}}, []string{"b", "a"}),
		[]*column{{name: "a"}, {name: "b", movedFrom: 0}})
	assert.Equal(t,
		detectMovedColumns(
			[]*column{{name: "a"}, {name: "b"}, {name: "c"}},
			[]string{"c", "a"},
		),
		[]*column{{name: "a"}, {name: "b"}, {name: "c", movedFrom: 0}})
	assert.Equal(t,
		detectMovedColumns([]*column{{name: "a"}, {name: "b"}}, []string{"b", "c", "a"}),
		[]*column{{name: "a"}, {name: "b", movedFrom: 0}})
	assert.Equal(t,
		detectMovedColumns(
			[]*column{
