...
func (s *testSuite) TestAuthenticate(t *testing.T) {
	...
	assert.Error(t, err)
	_, err = cli.Commit("alpha", "initial commit", "file.csv", bytes.NewReader(buf.Bytes()), nil, apiclient.WithAuthorization(tok))
	assert.Error(t, err)
	...
	assert.Error(t, err)
	_, err = cli.GetBlocks(com.Table, 0, 0, payload.BlockFormatCSV, false, apiclient.WithAuthorization(tok))
	assert.Error(t, err)
	...
	resp, err := cli.GetBlocks(com.Table, 0, 0, payload.BlockFormatCSV, false, apiclient.WithAuthorization(tok))
	require.NoError(t, err)
	...
	cr, err := cli.Commit("alpha", "initial commit", "file.csv", bytes.NewReader(buf.Bytes()), nil, apiclient.WithAuthorization(tok))
	require.NoError(t, err)
