...
func (h *Handler) handleAuthorize(rw http.ResponseWriter, r *http.Request) {
	...
	if s := values.Get("response_type"); s != "code" {
		outputError(rw, &Oauth2Error{"invalid_request", "response_type must be code"})
		return
	}
	...
	for _, key := range []string{
		...
		if s := values.Get(key); s == "" {
			outputError(rw, &Oauth2Error{"invalid_request", fmt.Sprintf("%s required", key)})
			return
		}
	}
	if s := values.Get("code_challenge_method"); s != "S256" {
		outputError(rw, &Oauth2Error{"invalid_request", "code_challenge_method must be S256"})
		return
	}
	redirectURI := values.Get("redirect_uri")
	if !h.validRedirectURI(clientID, redirectURI) {
		outputError(rw, &Oauth2Error{"invalid_request", "invalid redirect_uri"})
		return
	}
	if _, err := url.Parse(redirectURI); err != nil {
		outputError(rw, &Oauth2Error{"invalid_request", fmt.Sprintf("invalid redirect_uri: %v", err)})
		return
	}
